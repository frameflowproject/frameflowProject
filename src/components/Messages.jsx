...
const CallModal = ({
  ...
  useEffect(() => {
    if (isOpen && callStatus === "incoming") {
      ...
      ringtoneRef.current
        .play()
        .catch((e) => console.warn("Ringtone blocked:", e));
    } else {
  ...
  const handleRemoteTrack = useCallback((event) => {
    ...
    if (event.track.kind === "video") {
      ...
      addLog("Video track ready");
    }

    // Use the stream from the event if available, otherwise create new
    if (event.streams && event.streams[0]) {
      console.log("📡 Using stream from event:", event.streams[0]);
      setRemoteStream(event.streams[0]);
    } else {
      // Fallback: manually construct stream
      console.log("📡 Manually constructing stream");
  ...
  const startCall = async () => {
    ...
    try {
      ...
      try {
        ...
        addLog("No Mic/Video");
        return; // Exit if we can't get media
      }

      // 2. Add Transceivers and Tracks
      ...
      if (isIncoming && callerSignal) {
        ...
        console.log("📤 Sending answer with tracks:", {
          audio: stream.getAudioTracks().length,
          video: stream.getVideoTracks().length,
        });
        ...
        console.log("📤 Sending offer with tracks:", {
          audio: stream.getAudioTracks().length,
          video: stream.getVideoTracks().length,
        });
    ...
    if (socket && isOpen) {
      const onAnswer = async (data) => {
        ...
        if (peerRef.current && !peerRef.current.currentRemoteDescription) {
          ...
          setCallStatus("connected");
          processIceQueue();
          console.log("✅ Call connected - waiting for tracks");
        }
      };
    ...
    >
        ...
        >
          <div
            style={{
              ...
              height: "140px",
              borderRadius: "50%",
              background: otherUser?.avatar
                ? `url(${otherUser.avatar}) center/cover`
                : "#7c3aed",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
          ...
          <h2
            style={{ color: "white", fontSize: "2rem", marginBottom: "40px" }}
          >
            {otherUser?.fullName}
          </h2>
      ...
      <div
        style={{
          ...
          marginTop: "60px",
          zIndex: 10,
          opacity: callStatus === "incoming" ? 0 : 1,
        }}
      >
        ...
        <p
          style={{
            color:
              callStatus === "connected" ? "#22c55e" : "rgba(255,255,255,0.8)",
            fontSize: "1.1rem",
            fontWeight: "500",
...
const Messages = () => {
  ...
  const getCallerDetails = () => {
    if (incomingCall) {
      ...
      const conv = conversations.find(
        (c) => c.participant.id === incomingCall.userCalling,
      );
  ...
  const handleSendMessage = async () => {
    ...
    const currentFollowing = (user?.following || []).map((f) =>
      String(f._id || f),
    );
    if (
      !currentFollowing.includes(String(selectedConversation.participant.id))
    ) {
  ...
  const conversationMessages = selectedConversation?.participant.id
    ? getConversationMessages(selectedConversation.participant.id).filter(
        (m) => !deletedMessages.includes(m.id),
      )
    : [];
  ...
  const addReaction = (messageId, reaction) => {
    setMessageReactions((prev) => ({
      ...prev,
      [messageId]: [
        ...(prev[messageId] || []).filter((r) => r.userId !== user?.id),
  ...
  const handleCoWatchInvite = () => {
    ...
    navigate(
      `/videos?cowatch=true&friendId=${selectedConversation.participant.id}`,
    );
  };
  ...
  const filteredConversations = Array.isArray(conversations)
    ? conversations.filter((conv) => {
        ...
        const isFollowing = (user?.following || []).some(
          (f) => String(f._id || f) === String(conv.participant.id),
        );
        ...
        if (filterMode === "favorites")
          return matchesSearch && favorites.includes(conv.id);
  ...
  if (selectedConversation) {
    ...
    return (
      <div
        style={{
          height: isDesktop ? "100vh" : "calc(100vh - 60px)",
          display: "flex",
          flexDirection: "column",
      ...
      >
        ...
        <CallModal
          ...
          user={
            incomingCall ? getCallerDetails() : selectedConversation.participant
          }
          callType={callType}
          ...
          >
            <div
              style={{
                ...
                background: selectedConversation.participant.avatar
                  ? `url(${selectedConversation.participant.avatar}) center/cover`
                  : userColor,
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
            ...
            >
              ...
              <span
                style={{
                  fontSize: "0.7rem",
                  color: isUserOnline(selectedConversation.participant.id)
                    ? "#22c55e"
                    : "var(--text-muted)",
                  whiteSpace: "nowrap",
                }}
        ...
        {showMessageSearch && (
          ...
          >
            ...
            {messageSearch && (
              ...
              >
                {displayMessages.length} result(s) found
              </p>
            )}
          </div>
        )}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 16px" }}>
          {displayMessages.length === 0 && !messageSearch && (
            <div style={{ textAlign: "center", padding: "40px 20px" }}>
              ...
              <h3 style={{ color: "var(--text)", marginBottom: "4px" }}>
                {selectedConversation.participant.fullName}
              </h3>
              <p
                style={{
              ...
              <div key={message.id || message.tempId}>
                ...
                <div
                  style={{
                    ...
                    gap: "8px",
                    marginBottom: "4px",
                    paddingLeft: isMe ? "50px" : "0",
                    paddingRight: isMe ? "0" : "50px",
                  }}
                >
                  {!isMe && (
                    <div
                      ...
                      >
                        ...
                        >
                          {message.replyTo.senderUsername === user?.username
                            ? "You"
                            : message.replyTo.senderFullName ||
                          ...
                          {message.text.includes("?cowatch=true") ? (
                            <div>
                              ...
                              <button
                                onClick={() => {
                                  const urlMatch =
                                    message.text.match(/http[s]?:\/\/[^ ]+/);
                                  if (urlMatch) {
                                    const url = new URL(urlMatch[0]);
                                    const roomId =
                                      url.searchParams.get("roomId");
                                    navigate(
                                      `/videos?cowatch=true&roomId=${roomId}`,
                                    );
                                  }
                                }}
                                style={{
  ...
  return (
    <div
      style={{
        minHeight: isDesktop ? "100vh" : "calc(100vh - 60px)",
        background: "var(--background)",
        ...
        >
          <div
            style={{
              ...
              animation:
                connectionStatus === "error" ? "none" : "pulse 1.5s infinite",
            }}
          />
            ...
            {["primary", "requests"].map((folder) => (
              <button
                ...
                style={{
                  ...
                  border: "none",
                  background:
                    activeFolder === folder ? "var(--primary)" : "transparent",
                  color:
                    activeFolder === folder ? "white" : "var(--text-secondary)",
                  cursor: "pointer",
                  fontSize: "0.8rem",
                  fontWeight: "600",
            ...
            {["all", "favorites", "unread"].map((mode) => (
              <button
                ...
                style={{
                  ...
                  border: "none",
                  background:
                    filterMode === mode ? "var(--primary)" : "transparent",
                  color:
                ...
                >
                  <div
                    style={{
                      ...
                      background: conversation.participant.avatar
                        ? `url(${conversation.participant.avatar}) center/cover`
                        : userColor,
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      ...
                      fontSize: "1.3rem",
                      fontWeight: "600",
                      border: `3px solid ${isOnline ? "#22c55e" : "var(--border-color)"}`,
                    }}
                  >
                  ...
                  <div>
                    ...
                    >
                      {isOnline
                        ? "● Online"
                        : `Last seen ${formatTime(conversation.lastMessage?.timestamp)}`}
                    </p>
                  </div>
                </div>
                  ...
                  >
                    <span
                      style={{
                        fontSize: "0.95rem",
                        fontWeight:
                          conversation.unreadCount > 0 ? "600" : "500",
                        color: "var(--text)",
                      }}
                    >
                      {conversation.participant.fullName}
