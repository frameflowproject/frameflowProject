...
const SuggestedUsers = ({ className = "" }) => {
        ...
        const transformedUsers = data.suggestions
          ...
          .map((user) => {
            return {
              ...
              recommendationReason: user.recommendationReason || {
                type: "discover",
  ...
  const fetchFollowStatusForUsers = async (users, token) => {
    try {
      const followStatusPromises = users.map(async (user) => {
        try {
          const response = await fetch(
            `${import.meta.env.VITE_API_URL}/api/users/follow-status/${user.id}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
          ...
          console.error(
            `Error fetching follow status for user ${user.id}:`,
            error,
          );
          return {
            userId: user.id,
  ...
  if (loading) {
    return (
      <div
        className={`suggested-users ${className}`}
        style={{
          ...
          {error.includes("Invalid token") && (
            <button
              onClick={async () => {
                ...
                const testToken =
                  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2OTNjMjkwZGFiNDc0Y2I1MmQ5MWJmOWYiLCJpYXQiOjE3NjY4MTExODQsImV4cCI6MTc2NzQxNTk4NH0.DYgYbLW7LMRC1yGz5bVuGAjCzplVgrHbQ2xfMsP1sDI";
                const testUser = {
      ...
      >
        ...
        >
          ...
          <p style={{ margin: 0, fontSize: "0.9rem" }}>
            No trending creators this week
          </p>
        </div>
      </div>
    );
